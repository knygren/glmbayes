---
title: "Chapter 4: Specifying Multivariate Normal Priors and Interpreting Model Outputs"
author: "Kjell Nygren"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Chapter 4: Specifying Multivariate Normal Priors and Interpreting Model Outputs}
...
- id: nygren2006
  ...
  volume: 101
  URL: 'https://www.jstor.org/stable/27590791'
...
*Interpreting the "Prior and Maximum Likeihood Estimates with Standard Deviations" table*
...
We close this chapter with a discussion of a utility function provided as part of this package that allows the user to check for prior-data conflicts prior to estimation of the Bayesian model. 
...
To catch some of these cases without influencing potentially good prior specifiations un-necessarily we propose the following approach that involes running a classical model with the same specification without directly viewing the coefficient estimates and then passing information related to the classical estimates together with the  prior specification to our utility function so check if the maximum likelihood estimates are consistent with the prior.

Our utility function *Prior_Likelihood_Check" is set up to check for consistency using the following steps for each coefficient

* Check if the maximum likelihood estimate is outside of the prior credible interval at the specified confidence level (default 95%).
* If this is the case, check if the maximum likelihood estimate based confidence interval for the same level of confidence overlaps with the credible interval and flag if it does not.
* Print a warning if some coefficients are flagged under the second tests and recommend for the user to run a summary for the output from the utlity function to investigate further.

Conceptually, this approach is similar to that described in [@Evans2006]. 

...
#'
#' \code{rnorm_gamma_reg_temp3} is used to generate iid samples from Bayesian linear models with a normal-gamma prior. 
...
rnorm_gamma_reg_temp3<-function(n,y,x,mu,P,nu,V,offset2=NULL,wt=1){
...
## a_prior --> Prior shape parameter
## b_prior --> Prior rate parameter in gamma distribution (scale in inverse gamma)

## Do a complete rewrite (see Bayesian Regression Wikipedia for now)
## https://en.wikipedia.org/wiki/Bayesian_linear_regression

## Might want more efficient and accurate method for these
## For now use for transparency

## need to subtract offset from y and adjust for the weights

P_Post=P+t(x)%*%x
mu_Post=solve(P_Post)%*%(mu+t(x)%*%y)
Var_Post1=solve(P_Post)

###  Parameters

a_prior=2*nu     ## Should be relationship to shape in Wishart  
b_prior=1/V  ## Should be relationship to scale in Wishart

a_post=a_prior+(nobs/2) # Posterior Shape parameter
b_post=b_prior+(t(y)%*%y + t(mu)%*%P%*%mu - t(mu_Post)%*%P_Post%*%mu_Post) # Posterior rate
...
## Can likely call rnorm_reg_cpp here in a loop is we adjust mu and/or P for the dispersion first
## P should likely be divided by the dispersion
## should mu also be adjusted? 
## X and y are both divided by sqrt(dispersion)
## (P/dispersion)*mu + (X^{T}/sqrt{dispersion) (y/sqrt{dispersion})= 
## =(1/dispersion} [P mu + X^{T} y]
## Should be enough to divide P and pass dispersion (so it adjusts) X and y 

out1<-matrix(0,nrow=n,ncol=nvars)


for(i in 1:n){
P_Temp= P/dispersion[i]

## Now simulate from multivariate normal


  outlist=rnorm_reg_cpp(
  n=1,
  y=y,
  x=x,
  mu=mu,
  ...
  out[i,]=as.matrix(outlist$coefficients,nrow=1)
}
...
for(i in 1:n){
    ...
    LL[i]=f1(b=out[i,],y=y,x=x,alpha=offset2,wt=wt/dispersion[i])	
}


outlist2<-list(
  coefficients=out
  ,PostMode=outlist$PostMode, # This should work as PostMode same regardless of iteration
              Prior=list(mean=as.numeric(mu),Precision=P),
              iters=draws,famfunc=famfunc,Envelope=NULL,
              dispersion=dispersion,loglike=LL)

colnames(outlist2$coefficients)<-colnames(x)

outlist2$call<-match.call()

class(outlist2)<-c(outlist2$class,"rglmb")

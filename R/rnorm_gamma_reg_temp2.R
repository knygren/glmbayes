...
#'
#' \code{rnorm_gamma_reg_temp2} is used to generate iid samples from Bayesian linear models with a normal-gamma prior. 
...
rnorm_gamma_reg_temp2<-function(n,y,x,mu,P,nu,V,offset2=NULL,wt=1){
...
a_prior=nu/2     ## Should be relationship to shape in Wishart  
b_prior=1/(2*V)  ## Should be relationship to scale in Wishart

a_post=a_prior+(nobs/2) # Posterior Shape parameter
b_post=b_prior+(t(Ytemp)%*%Ytemp + t(mu)%*%P%*%mu - t(mu_Post)%*%P_Post%*%mu_Post) # Posterior rate

dispersion=1/rgamma(n=n,shape=a_post,rate=b_post)

for(i in 1:n){
  
  V_S_Inverse=chol2inv(chol(V+S))
  rwout=rwishart(nu+l1,V_S_Inverse)
  ...
  if(m==1){
    ...
    # Old function returned IW. Modified in two steps
    
#    out2[i,1]<-rwout$IW  ## Old function Returned value from Inverse Wishart

    ###  Modification 1: Return Wishart Draw and then invert before passing to final output

    W=rwout$W
    IW=solve(W)

    out2[i,1]<-IW  ## Should match original function
    
      }
  else{

#    out2[i]<-rwout$IW    ## Returned value from Inverse Wishart

    ### Modification 2: 
        
    W=rwout$W
    IW=solve(W)
    out2[i,1]<-IW  ## Should match orginal function

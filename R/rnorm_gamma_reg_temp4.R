...
#'
#' \code{rnorm_gamma_reg_temp4} is used to generate iid samples from Bayesian linear models with a normal-gamma prior. 
...
rnorm_gamma_reg_temp4<-function(n,y,x,mu,P,nu,V,offset2=NULL,wt=1){
...
b_post=b_prior+0.5*(t(Ytemp)%*%Ytemp + t(mu)%*%P%*%mu - t(mu_Post)%*%P_Post%*%mu_Post) # Posterior rate

## Estimate just dispersion first - make sure is working
dispersion=1/rgamma(n=n,shape=a_post,rate=b_post)


return(list(n=n,a_post=a_post,b_post=b_post))
